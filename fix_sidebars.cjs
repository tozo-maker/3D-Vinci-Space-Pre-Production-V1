...
// RootLayout.tsx
let root = fs.readFileSync('src/RootLayout.tsx', 'utf8');
root = root.replace(/<div className="fixed top-0 left-0 w-16 h-screen z-40 hidden xl:flex flex-col justify-between items-center py-10">/, '<div className="fixed top-0 left-0 w-16 h-screen z-40 hidden xl:flex flex-col justify-between items-center py-10 bg-brand-bg/80 backdrop-blur-md">');
root = root.replace(/<div className="flex flex-col items-center gap-6 mt-auto mix-blend-difference pointer-events-none">/, '<div className="flex flex-col items-center gap-6 mt-auto pointer-events-auto">');
root = root.replace(/<span className="font-mono text-\[9px\] uppercase tracking-\[0\.4em\] text-white whitespace-nowrap">/, '<span className="font-mono text-[9px] uppercase tracking-[0.4em] text-brand-muted whitespace-nowrap">');
fs.writeFileSync('src/RootLayout.tsx', root);

// SectionNav.tsx
const navContent = `import React, { useEffect, useState } from 'react';
...
fs.writeFileSync('src/components/SectionNav.tsx', navContent);
console.log("Sidebars fixed.");
